...
extension KiaEuropeAPIClient {
    ...
    func authorizedHeaders(authToken: AuthToken, ccs2: Bool = false) -> [String: String] {
        [
            ...
            // Fresh Stamp per request — the gateway validates the embedded
            // timestamp window.
            "Stamp": generateStamp()
        ]
    }
    ...
    /// XOR of the base64-decoded `authCfb` with `<appId>:<unix seconds>`,
    /// base64-encoded — the CCSP gateway validates this and returns
    /// HTTP 400 for anything else. Matches `_get_stamp()` in
    /// hyundai_kia_connect_api.
    func generateStamp() -> String {
        let raw = Data("\(Self.appId):\(Int(Date().timeIntervalSince1970))".utf8)
        guard let cfbData = Data(base64Encoded: Self.authCfb) else { return "" }
        let xored = zip(cfbData, raw).map { $0 ^ $1 }
