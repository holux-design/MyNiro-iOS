...
public final class KiaEuropeAPIClient: APIClientBase, APIClientProtocol {
    ...
    /// Last successful CCS2/legacy modem wake (force refresh).
    var lastRealTimeRefresh: Date?

    var baseURL: String {
        region.apiBaseURL(for: .kia)
    }
    var authBaseURL: String { "https://idpconnect-eu.kia.com" }
    var apiHost = Brand.kiaBaseUrl(region: Region.europe).replacingOccurrences(of: "https://", with: "")
    ...
    public func login() async throws -> AuthToken {
        ...
        // The CCAPI gateway returns HTTP 400 on every /spa endpoint when
        // ccsp-device-id is missing or unregistered, so make sure we have
        // a registered device before handing out a token.
        if configuration.deviceId?.isEmpty != false {
    ...
    public override func registerDevice() async throws -> String? {
        // Random 64-hex push registration id, matching _get_device_id()
        // in hyundai_kia_connect_api.
        let pushRegId = (0..<32)
    ...
    public func fetchVehicleStatus(
        ...
        return try await forceRefreshVehicleStatus(for: vehicle, authToken: authToken)
    }
