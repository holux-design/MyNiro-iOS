...
extension KiaEuropeAPIClient {
    ...
    func signin() async throws -> String {
        ...
        let location = try await submitSignin(encryptedHex: encryptedHex, kid: jwk.kid)
        return try parseSigninCode(fromLocation: location)
    }
    ...
    private func fetchAuthorizeCookies() async throws {
        ...
        let (_, response) = try await urlSession.data(for: authorizeReq)
        if let http = response as? HTTPURLResponse, !(200...399).contains(http.statusCode) {
            throw APIError(
                message: "Authorize failed: HTTP \(http.statusCode). Kia may be blocking this client.",
                apiName: apiName,
    ...
    private func fetchSigninJWK() async throws -> KiaEuropeJWK {
        ...
        let (certsData, certsResp) = try await urlSession.data(for: certsReq)
        if let http = certsResp as? HTTPURLResponse, http.statusCode != 200 {
            let snippet = String(data: certsData, encoding: .utf8)?.prefix(200) ?? ""
            throw APIError(
                message: "Failed to fetch RSA certs: HTTP \(http.statusCode) — \(snippet)",
                apiName: apiName,
                errorType: .serverError
            )
        }
    ...
    /// Step 3: POST signin (form-encoded). Must NOT follow the 302 — the
    /// `Location` header carries `?code=…`. Following it hits CCAPI and
    /// returns WAF 400 ("classified as an abusing request").
    private func submitSignin(encryptedHex: String, kid: String) async throws -> String {
        ...
        let (data, signinResp) = try await EUAuthSession.noRedirect.data(for: signinReq)
        guard let http = signinResp as? HTTPURLResponse else {
            throw APIError.invalidCredentials("Signin returned no HTTP response", apiName: apiName)
        }
        ...
        if http.statusCode == 400 || body.localizedCaseInsensitiveContains("abusing") {
            throw APIError(
                message: "Kia blocked the login request (HTTP \(http.statusCode)). "
                    + "This is usually a WAF false positive — try again in a minute. \(body)",
                apiName: apiName,
                errorType: .serverError
            )
        }
        throw APIError.invalidCredentials(
            "Signin failed: HTTP \(http.statusCode) — \(body). Check email and password.",
            apiName: apiName
        )
    }

    /// Parse the 302 Location query for `code` (success) or translate an
    /// error redirect into a typed `APIError`.
    private func parseSigninCode(fromLocation location: String) throws -> String {
        guard let url = URL(string: location),
              let comps = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
            throw APIError.invalidCredentials(
                "Signin redirect was not a valid URL: \(location.prefix(200))",
                apiName: apiName
            )
        }
        ...
        if comps.path.contains("/web/v1/user/authorization") {
            throw APIError(
                message: "Kia account consent required — log in via the official app once to accept the terms, then try again.",
                apiName: apiName
            )
        }
        if comps.path.contains("authorize") || location.contains("authorize") {
            throw APIError.invalidCredentials(
                "Authentication failed — returned to login page. Check email and password.",
                apiName: apiName
            )
        }
        throw APIError(
            message: "Unexpected redirect after signin: \(location.prefix(250))",
            apiName: apiName
        )
    }
    ...
    private func postTokenRequest(fields: [(String, String)], isRefresh: Bool) async throws -> AuthToken {
        ...
        if let http = response as? HTTPURLResponse, http.statusCode != 200 {
            ...
            throw APIError.invalidCredentials(
                "Token exchange failed: HTTP \(http.statusCode) — \(snippet)",
                apiName: apiName
            )
        }
